...
public interface BaseNodeInterface {
	 ...
	 * 
	 * @param baseNodeId The unique id of an existing node.
	 * @param name The name of a node must be unique among its sibling-nodes.
	 * @throws UnauthorizedException If the authorized user is not Admin on
	 * 		the parent-node of the given node. Toplevel nodes can only be
	 ...
	 * 
	 * @param baseNodeId The unique id of an existing node.
	 * @param displayName A string which is typically used instead of
	 * 		the <em>name</em> in user-interfaces.
	 * @throws UnauthorizedException If the authorized user is not Admin on
	 * 		the parent-node of the given node. Toplevel nodes can only be
	 * 		modified by a SuperAdmin.
	 * @throws NoSuchObjectException If no node with the given id exists.
	 ...
	 * @throws UnauthorizedException If the authorized user is not Admin on
	 * 		the parent-node of the given node. Toplevel nodes can only be
	 * 		removed by a SuperAdmin.
	 * @throws NotEmptyException If the given node has any child-nodes. This
	 * 		exception is never thrown if the authorized user is SuperAdmin
	 * 		because SuperAdmin can remove non-empty nodes.
	 * @throws NoSuchObjectException If no node with the given id exists. 
	 */
	...
	/** Get id from path.
	 * 
	 * @param nodePath The path of a node.
